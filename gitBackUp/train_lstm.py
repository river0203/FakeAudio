...
# LSTM 모델 학습 (전체 MFCC와 LFCC 값 사용)
mfcc_lfcc_indices = list(range(52))  # 13 (MFCC mean) + 13 (LFCC mean) + 26 (flattened MFCC and LFCC)

X_train_lstm = X_train[:, mfcc_lfcc_indices]
X_test_lstm = X_test[:, mfcc_lfcc_indices]

model = tf.keras.Sequential([
    tf.keras.layers.Reshape((X_train_lstm.shape[1], 1), input_shape=(X_train_lstm.shape[1],)),
